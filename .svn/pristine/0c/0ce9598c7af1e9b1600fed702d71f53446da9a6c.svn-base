...
class WC_Cancel_Order{
	...
	function wc_cancel_save_settings(){
		if(isset($_POST['wc-cancel'])){
			update_option('wc_cancel_settings',$_POST['wc-cancel'],'no');
		}
		$this->load_settings();
	}
	...
	function load_settings(){
		...
		$this->settings = array(
			...
			'text-required' => isset($pro_settings['text-required']) ? $pro_settings['text-required'] : 1,
				...
				if(is_a($order,'WC_Order')){
					...
					if(($this->user_has_role() && $this->check_order_customer($order)) || ($this->user_has_role() && $order_id_by_key==$order_id)){
						if(isset($_REQUEST['additional_details'])){
							update_post_meta($order_id,'_wc_cancel_additional_txt',$_REQUEST['additional_details']);
						}
