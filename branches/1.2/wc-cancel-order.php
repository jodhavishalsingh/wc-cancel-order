...
class WC_Cancel_Order {
	...
	public function add_wc_cancel_email_actions($actions){
	...
	public function approve_cancel_request($id,$status){
		   ...
		   if($get_count>0){
			...
			$wpdb->update($wpdb->prefix."wc_cancel_orders",array('is_approved'=>1,'cancel_date'=>current_time('mysql')), 
	...
	public function reject_cancel_request($id,$status){
		   ...
		   if($get_count){
			...
			$wpdb->update($wpdb->prefix."wc_cancel_orders",array('is_approved'=>2,'cancel_date'=>current_time('mysql')), 
...
// Add to list of WC Order statuses
	...
	public function mark_order_cancelled() {
		...
		wp_safe_redirect( wp_get_referer() ? wp_get_referer() : admin_url('page=wc_cancel'));
	...
	public function add_wc_cancel_my_account_orders_status($actions,$order){
		...
		$actions['cancelled'] = array(
			...
			'url' 		=> wp_nonce_url(admin_url('admin-ajax.php?action=mark_order_as_cancell_request&order_id='.$order->id),'woocommerce-mark-order-cancell-request-myaccount'),

			'name' 		=> __('Send Cancel Request','woocommerce'),
